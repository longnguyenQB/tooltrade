...
def main():
    ...
    def trade_sell(price_sell):
        ...
        for price_buy in np.arange(price_sell , price_sell-0.05, -0.0001)[1:]:
            for j in np.arange(5, 20, 5): 
                number_token_sell = (j*price_buy*(1 + transaction_fee)/(price_sell*(1 - transaction_fee)-price_buy*(1 + transaction_fee)))
                if (number_token_sell < 2000) & (number_token_sell > 0):
    ...
    def trade_buy(price_buy):
        ...
        for price_sell in np.arange(price_buy , price_buy + 0.05, 0.001)[1:]:
            for j in np.arange(5, 20, 5): 
                number_token_buy = (j*price_sell*(1 - transaction_fee)/(price_sell*(1 - transaction_fee)-price_buy*(1 + transaction_fee)))
                if (number_token_buy < 2000) & (number_token_buy >0):
